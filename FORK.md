...
Current base: **`52a412d9`** (release tag `v26.9.9`, Go 1.27). The fork's binary is cut
as release **`v26.9.9-perinbound1`** on `kirisame-meguru/xray-core` — this tag must equal node's
...
The 3.4.4 stack sync preserves the existing `DispatchLink` limitation: per-inbound downlink is
counted, but uplink on that path is not. The real VLESS smoke check verified opt-in, exact downlink
bytes, opt-out, and counter reset; `getLink` retains both directions. No tracking behavior changed.
