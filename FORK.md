...
Current base: **`c320e891`** (`xtls/main`, 15 commits past tag `v26.6.27`). The fork's binary is cut
as release **`v26.6.27-perinbound2`** on `kirisame-meguru/xray-core` — this tag must equal node's
`Dockerfile` `ARG XRAY_CORE_VERSION`. Bump the `-perinboundN` suffix (and re-cut) whenever the XTLS
base moves or the feature commit changes.
