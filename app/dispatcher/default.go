...
// emitted for the given inbound tag (i.e. the inbound set trackTrafficPerUser and
// so its tag landed on the tracked allowlist).
...
func (d *DefaultDispatcher) getLink(ctx context.Context) (*transport.Link, *transport.Link) {
	...
	if user != nil && len(user.Email) > 0 {
		...
		// above. Gated solely by the inbound-tag allowlist, which infra/conf
		// derives from the inbounds' trackTrafficPerUser flag, so this never
		// alters existing per-user stats wiring.
...
func WrapLink(ctx context.Context, policyManager policy.Manager, statsManager stats.Manager, trackedInboundTags map[string]bool, link *transport.Link) *transport.Link {
	...
	if user != nil && len(user.Email) > 0 {
		...
		// emitted only via getLink (the primary connection path).
		if len(sessionInbound.Tag) > 0 && trackedInboundTags[sessionInbound.Tag] {
			name := "useri>>>" + user.Email + ">>>inbound>>>" + sessionInbound.Tag + ">>>traffic>>>downlink"
			if c, _ := stats.GetOrRegisterCounter(statsManager, name); c != nil {
				link.Writer = &SizeStatWriter{
					Counter: c,
